...
const Hero = () => {
  ...
  return (
    <header id="about" className="relative max-w-6xl mx-auto px-6 py-20 md:py-32 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row items-center gap-12 md:gap-20">
        ...
        <div className="flex-1 space-y-8 text-center md:text-left">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
            <a href="#projects" className="px-8 py-4 bg-teal-600 text-white rounded-xl font-semibold hover:bg-teal-700 transition-all hover:scale-105 hover:shadow-xl shadow-teal-200/50 dark:shadow-none">
              View My Projects
            </a>
            <a href={personal.githubUrl} target="_blank" rel="noreferrer" className="px-8 py-4 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-800 rounded-xl font-semibold hover:bg-slate-50 dark:hover:bg-slate-800 transition-all hover:scale-105 flex items-center justify-center gap-2">
              <GitHub fontSize="small" /> GitHub
            </a>
          </div>
        </div>
        
        {/* Code Visual - Added Glassmorphism and better syntax highlighting */}
        <div className="w-full md:w-[45%] relative hidden md:block perspective-1000">
          ...
          <div className="relative bg-slate-900 dark:bg-[#0a0a0a] rounded-xl border border-slate-800 shadow-2xl overflow-hidden transform rotate-y-12 hover:rotate-0 transition-all duration-700 ease-out">
            ...
            <div className="flex items-center gap-2 px-4 py-3 bg-slate-800/50 border-b border-slate-800">
              ...
              <div className="ml-auto text-xs text-slate-500 font-mono">Developer.java</div>
            </div>
            
            {/* Code Content */}
            <div className="p-6 space-y-2 font-mono text-xs sm:text-sm text-slate-300 leading-relaxed">
                <p><span className="text-purple-400">public class</span> <span className="text-yellow-300">Portfolio</span> <span className="text-purple-400">extends</span> <span className="text-yellow-300">Developer</span> {'{'}</p>
                <div className="pl-4 border-l-2 border-slate-800 ml-1">
                  <p className="text-slate-500">// Tech Stack</p>
                  <p>String[] skills = {'{'}</p>
                  <p className="pl-4"><span className="text-emerald-400">"Java"</span>, <span className="text-emerald-400">"Spring Boot"</span>,</p>
                  <p className="pl-4"><span className="text-emerald-400">"React"</span>, <span className="text-emerald-400">"PostgreSQL"</span></p>
                  <p>{'};'}</p>
                  <br/>
                  <p><span className="text-blue-400">public void</span> <span className="text-yellow-300">hireMe</span>() {'{'}</p>
                  <p className="pl-4">System.out.println(<span className="text-emerald-400">"Ready to work!"</span>);</p>
                  <p>{'}'}</p>
                </div>
                <p>{'}'}</p>
            </div>
